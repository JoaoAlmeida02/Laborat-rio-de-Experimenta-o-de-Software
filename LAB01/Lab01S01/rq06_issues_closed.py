...
# busca os 100 repositorios mais populares do GitHub e coleta o total de issues abertas e fechadas de cada um.
...
OUTPUT_CSV = os.path.join(DATA_DIR, "rq06_issues.csv")
MAX_RETRIES = 3
...
def extract_metric(repo: dict) -> dict:
    ...
    # evita divisao por zero em repositorios sem nenhuma issue

    ratio = round(issues_closed / total_issues, 3) if total_issues > 0 else "N/A"
...
def save_csv(rows: list[dict], path: str) -> None:
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["repository", "stars", "issues_open", "issues_closed", "issues_closed_ratio"],
        )
        writer.writeheader()
        writer.writerows(rows)

def print_validation_sample(rows: list[dict], sample_size: int = 10) -> None:
    print(f"\nAmostra de validação ({sample_size} primeiros repositorios):")
    print(f"\n{'Repositorio':<40}{'Abertas':>10}{'Fechadas':>10}{'Razao':>10}")
    for row in rows[:sample_size]:
        print(
            f"{row['repository']:<40}{row['issues_open']:>10}"
            f"{row['issues_closed']:>10}{str(row['issues_closed_ratio']):>10}"
        )


def print_summary(rows: list[dict]) -> None:
    ratios = [row["issues_closed_ratio"] for row in rows if row["issues_closed_ratio"] != "N/A"]
    na_count = len(rows) - len(ratios)

    print("\nResumo (RQ06 - razao issues fechadas/total): ")
    print(f"Repositorios analisados: {len(rows)}")
    print(f"Sem nenhuma issue (N/A): {na_count}")
    if ratios:
        print(f"Mediana: {statistics.median(ratios):.3f}")
        print(f"Media: {statistics.mean(ratios):.3f}")
        print(f"Minimo: {min(ratios):.3f}")
        print(f"Maximo: {max(ratios):.3f}")
